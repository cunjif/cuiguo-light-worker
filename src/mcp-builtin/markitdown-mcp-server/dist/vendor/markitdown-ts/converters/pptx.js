...
export class PptxConverter {
    ...
    async convert(input, _info, opts) {
        ...
        if (authorsXml) {
            ...
            const authors = this.ensureArray(authorsDoc?.['p:cmAuthorLst']?.['p:cmAuthor'] ??
            ...
            for (const [, rel] of slideRelsMap) {
                if (rel.type.includes('comments')) {
                    ...
                    if (commentXml) {
                        ...
                        const comments = this.ensureArray(commentDoc?.['p:cmLst']?.['p:cm'] ??
                            commentDoc?.cmLst?.cm);
