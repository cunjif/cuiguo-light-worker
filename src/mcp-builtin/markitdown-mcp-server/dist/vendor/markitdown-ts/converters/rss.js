...
export class RssConverter {
    ...
    accepts(info) {
        ...
        if (PRECISE_EXTENSIONS.includes(ext))
            return true;
        ...
        if (CANDIDATE_EXTENSIONS.includes(ext))
            return true;
    ...
    async parseRss(doc) {
        ...
        for (const item of items) {
            ...
            if (description)
                md += await this.parseContent(description);
            if (content)
                md += await this.parseContent(content);
        }
    ...
    async parseAtom(doc) {
        ...
        for (const entry of entries) {
            ...
            if (summary)
                md += await this.parseContent(summary);
            if (content)
                md += await this.parseContent(content);
        }
