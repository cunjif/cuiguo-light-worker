...
import { UnsupportedFormatError, FileConversionError, } from './errors.js';
import { DEFAULT_MAX_BUFFER_SIZE, DEFAULT_MAX_UNCOMPRESSED_SIZE, PRIORITY_GENERIC, PRIORITY_SPECIFIC, } from './constants.js';
...
export class MarkItDown {
    ...
    async convert(source, options) {
        ...
        if (source.startsWith('http:') || source.startsWith('https:')) {
            if (!options?.allowUrlFetch) {
                throw new Error('URL fetching requires allowUrlFetch: true in options (SSRF protection). ' +
    ...
    buildLlmCaption() {
        if (this.options.llmCaption)
            return this.options.llmCaption;
    ...
    async dispatch(input, info, options) {
        ...
        for (const guess of guesses) {
            ...
            for (const reg of candidates) {
                if (!reg.converter.accepts(guess))
                    continue;
                anyAccepted = true;
                try {
    ...
    enableBuiltins() {
        ...
        if (this.options.docintelEndpoint) {
            try {
                ...
                this.registerConverter(new DocumentIntelligenceConverter({
