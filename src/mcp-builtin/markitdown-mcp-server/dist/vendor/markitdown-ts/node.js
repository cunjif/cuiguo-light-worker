 ...
 */
export function createFsReader() {
...
export function createExiftoolReader() {
    return async (input, info) => {
        ...
        try {
            ...
            throw new Error('exiftool-vendored is required for EXIF metadata extraction. Install with: pnpm add exiftool-vendored');
        }
        try {
 ...
 */
export function createAudioTranscriber(transcribeFn) {
    return async (input, info) => {
        if (!transcribeFn) {
            throw new Error('Audio transcription requires a transcribeFn callback. ' +
