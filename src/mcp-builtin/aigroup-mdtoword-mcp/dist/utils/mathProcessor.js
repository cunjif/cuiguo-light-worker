import { Math, MathRun, MathFraction, MathRadical, MathSuperScript, MathSubScript, MathSubSuperScript, MathSum, MathLimitUpper, MathLimitLower, MathFunction, MathSquareBrackets, MathRoundBrackets, MathCurlyBrackets, MathAngledBrackets } from 'docx';
/**
 * LaTeX数学公式解析器
 * 将LaTeX数学表达式解析为AST结构
 */
...
export class MathConverter {
     ...
     */
    static convertComponents(components, config) {
        const result = [];
     ...
     * 将单个数学组件转换为docx数学对象
     */
    static convertComponent(component, config) {
        switch (component.type) {
            case 'text':
            case 'run':
     ...
     * 创建数学文本运行对象
     */
    static createMathRun(text, config) {
...
export class MathProcessor {
    ...
    processMathInMarkdown(markdown) {
        ...
        processed = processed.replace(inlineRegex, (match, latex, offset) => {
            ...
            const isInMathBlock = mathBlocks.some(block => offset >= block.startIndex && offset < block.endIndex);
            if (!isInMathBlock) {
                mathBlocks.push({
                    latex: latex.trim(),
                    startIndex: offset,
     ...
     */
    convertLatexToDocx(latex, config) {
        try {
