...
export class TableProcessor {
     ...
     * @returns 表格数据
     */
    static fromCSV(csvData, options) {
        ...
        // 转换为表格数据
        const rows = records.map((row) => row.map(cell => ({
            content: cell
        })));
        return {
            rows,
    ...
    static fromJSON(jsonData, options) {
        ...
        // 创建数据行
        const dataRows = data.map(item => cols.map(col => ({
     ...
     * @returns 表格数据
     */
    static createWithMerge(rows, styleName) {
        return {
            rows,
     ...
     */
    static getPresetStyle(styleName) {
