import { Paragraph, TableOfContents, AlignmentType } from 'docx';
/**
 * 目录生成器类
 * 负责生成和管理文档目录
 */
export class TOCGenerator {
     ...
     * 添加标题条目
     */
    addHeading(level, text, pageNumber) {
     ...
     * 创建目录段落
     */
    static createTOC(config) {
        const { title = '目录', levels = [1, 2, 3], showPageNumbers = true, tabLeader = 'dot' } = config;
     ...
     * 创建目录标题段落
     */
    static createTOCTitle(config) {
        const { title = '目录', titleStyle } = config;
     ...
     * 验证目录配置
     */
    static validateTOCConfig(config) {
        const errors = [];
