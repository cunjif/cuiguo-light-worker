...
export class ImageProcessor {
    static DEFAULT_MAX_WIDTH = 600; // 默认最大宽度（缇）
    static DEFAULT_MAX_HEIGHT = 800; // 默认最大高度（缇）
    static DEFAULT_ASPECT_RATIO = 0.667; // 默认宽高比 (2:3)
    /**
     * 支持的图片格式
     */
    static SUPPORTED_FORMATS = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg', 'webp'];
    /**
     * 加载图片数据
     * @param src 图片路径
     * @param baseDir Markdown文件所在目录，用于解析相对路径
     */
    static async loadImageData(src, baseDir) {
        try {
     ...
     * 从Data URL识别图片类型
     */
    static getImageTypeFromDataUrl(src) {
     ...
     * 从URL识别图片类型
     */
    static getImageTypeFromUrl(src) {
        // 处理特殊域名
     ...
     * 考虑最大尺寸限制和宽高比保持
     */
    static calculateDimensions(originalWidth, originalHeight, imageStyle) {
     ...
     * 验证图片格式是否支持
     */
    static isSupportedFormat(type, allowedFormats) {
        if (!type)
            return false;
     ...
     * 创建占位符SVG
     */
    static createPlaceholderSvg(width, height, errorMessage, alt, src) {
     ...
     * 转换毫米到缇
     */
    static convertMillimetersToTwip(mm) {
        return Math.round(mm * 56.692);
    }
    /**
     * 转换缇到毫米
     */
    static convertTwipToMillimeters(twip) {
