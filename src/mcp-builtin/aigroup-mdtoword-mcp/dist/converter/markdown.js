...
// 使用新版docx API
import { Document, Packer, Paragraph, TextRun, HeadingLevel, Table, TableRow, TableCell, ImageRun, AlignmentType, Header, Footer, SimpleField, NumberFormat } from 'docx';
export class DocxMarkdownConverter {
    md;
    effectiveStyleConfig;
    errorHandler;
    tocGenerator;
    mathProcessor;
    baseDir; // Markdown文件所在目录，用于解析相对路径
    constructor(styleConfig, baseDir) {
    ...
    async createDocument(tokens, mathBlocks) {
        ...
        if (this.effectiveStyleConfig.tableOfContents?.enabled) {
            ...
            // 添加目录
            tocElements.push(TOCGenerator.createTOC(tocConfig));
            // 添加分页符
     ...
     * 创建页眉（从配置对象）
     */
    createHeaderFromConfig(headerConfig) {
        if (!headerConfig) {
            return new Header({
                children: []
            });
     ...
     * 创建页脚（从配置对象）
     */
    createFooterFromConfig(footerConfig) {
        if (!footerConfig) {
            return new Footer({
                children: []
            });
     ...
     * 获取页码格式
     */
    getPageNumberFormat(formatType) {
        // NumberFormat 枚举值映射
        const formatMap = {
    ...
    async processTokens(tokens, mathBlocks) {
        const children = [];
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            switch (token.type) {
                ...
                case 'ordered_list_close':
                    ...
                    break;
                case 'list_item_open':
                    listLevel = (token.attrs && token.attrs.find((attr) => attr[0] === 'level')?.[1]) || 0;
                    const itemContent = await this.processInlineContentAsync(tokens[i + 2], undefined, mathBlocks);
                    const listItem = this.createListItem(itemContent, orderedList, listLevel);
                    if (inList) {
                ...
                case 'blockquote_open':
                    ...
                    children.push(this.createBlockquote(blockquoteContent));
                    break;
                case 'fence':
                    ...
                    while ((match = imgRegex.exec(token.content)) !== null) {
                        ...
                        const imgToken = {
                            ...
                            attrGet: function (name) {
                                const attr = this.attrs.find((a) => a[0] === name);
    ...
    async processInlineContentAsync(token, headingLevel, mathBlocks) {
        const runs = [];
     ...
     * 获取文本样式
     */
    getTextStyle(headingLevel) {
        if (headingLevel) {
            const headingKey = `h${headingLevel}`;
            const headingStyle = this.effectiveStyleConfig.headingStyles?.[headingKey];
            if (headingStyle) {
                return {
                    font: headingStyle.font,
                    size: headingStyle.size,
    ...
    createListItem(content, ordered, level) {
     ...
     * 创建表格（旧方法，保持兼容）
     */
    createTableLegacy(rows) {
        if (rows.length === 0)
            return new Table({ rows: [] });
        const isHeaderRow = (index) => index === 0;
    ...
    createParagraphWithImages(content) {
        const normalStyle = this.effectiveStyleConfig.paragraphStyles?.normal;
        return new Paragraph({
            children: content,
            spacing: {
    ...
    async createImageRun(token) {
        ...
        try {
            ...
            if (!ImageProcessor.isSupportedFormat(imageType, imageStyle?.supportedFormats)) {
                ...
                return this.createPlaceholderImageRun(src, alt, title, `不支持的图片格式: ${imageType || '未知'}`, dimensions);
            }
            // 如果图片加载失败，创建占位符
            if (loadError || !imageData || !imageType) {
                console.log(`   ⚠️ 创建图片占位符...`);
                const dimensions = ImageProcessor.calculateDimensions(undefined, undefined, imageStyle);
                return this.createPlaceholderImageRun(src, alt, title, loadError || '图片加载失败', dimensions);
            }
            ...
            let imageRun;
            try {
                imageRun = new ImageRun(imageRunConfig);
    ...
    async createImageParagraph(token) {
        const imageStartTime = Date.now();
        try {
            const imageStyle = this.effectiveStyleConfig.imageStyles?.default;
            const src = token.attrGet('src');
            ...
            if (src.startsWith('data:')) {
                ...
                processedImageData = imageData; // Base64返回的是string
     ...
     * 创建占位符图片
     */
    createPlaceholderImageRun(src, alt, title, errorMessage, dimensions) {
        const svgBuffer = ImageProcessor.createPlaceholderSvg(dimensions.width, dimensions.height, errorMessage, alt, src);
    ...
    async extractTableData(tokens, startIndex, mathBlocks) {
        const rows = [];
        let currentRow = [];
        let i = startIndex + 1;
